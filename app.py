...
# Самый надежный и простой маршрут GET
@app.route('/api/probe', methods=['GET'])
def probe_number_simple():
    # Забираем номер телефона из параметров ссылки (?phone=...)
    phone_param = request.args.get('phone', '')
    clean_phone = re.sub(r'\D', '', phone_param)
    
    if not clean_phone or len(clean_phone) < 10:
        return jsonify({"status": "error", "message": "Введите корректный номер в формате 79991234567"}), 400
...
def home():
    return jsonify({"status": "online", "message": "Enigma Engine is fully ready."})
