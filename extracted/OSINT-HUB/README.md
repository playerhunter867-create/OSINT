...
The catalog runs on GitHub Pages without the API because the bundled `osint-framework/public/arf.json` is loaded directly. If the Sherlock API is unavailable, the username pages fall back to public profile links generated from the bundled site definitions; this fallback does not claim that a profile exists.
...
The frontend automatically tries a configured `localStorage.osint_api` URL and then a same-origin `/api` backend. If neither is available, static fallback mode remains usable.
