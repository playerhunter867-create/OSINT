...
async function init(){
  try{
    ...
    try {
      ...
      if(!r.ok) throw new Error('API catalog unavailable');
      d=await r.json();
    } catch (_) {
      const r=await fetch('osint-framework/public/arf.json');
      ...
      d={resources,resource_count:resources.length,category_count:cats.length,categories:cats};
    }
  ...
  }catch(e){$('grid').innerHTML=`<div class="empty">Could not load catalog.<br><small>${esc(e.message)}</small></div>`}
}
